...
class TestTextToTextNodes(unittest.TestCase):
    ...
    def test_only_link(self):
        new_nodes = text_to_textnodes("[link](https://boot.dev)")
        self.assertListEqual(
            [TextNode("link", TextType.LINK, "https://boot.dev")], new_nodes
        )

    def test_url_with_underscore_not_treated_as_italic(self):
        # regression: delimiter splitting must not run on image/link URLs
        text = "![alt](https://example.com/my_image.png)"
        new_nodes = text_to_textnodes(text)
        self.assertListEqual(
            [TextNode("alt", TextType.IMAGE, "https://example.com/my_image.png")],
            new_nodes,
        )

    def test_multiple_bold_and_italic(self):
        text = "**bold1** normal _italic1_ **bold2** normal _italic2_"
        new_nodes = text_to_textnodes(text)
        self.assertListEqual(
            [
